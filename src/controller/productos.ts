...
export class productosController {
  ...
  async hayProductos(req: Request, res: Response, next: NextFunction){
    const productos = await productsAPI.getInstance().getProducts();
  ...
  async productExists(req: Request, res: Response, next: NextFunction){
    const {id} = req.params;
    const producto = await productsAPI.getInstance().getProducts(id)
    if (!producto.length) {
      return res.status(404).json({
  ...
  */
  async getProducts(req: Request, res: Response) {
    ...
    if (id) {
      return res.json({
        data: await productsAPI.getInstance().getProducts(id),
      });
    }
    ...
    if(Object.keys(query).length){
      const productos = await productsAPI.getInstance().query(query);
    ...
    return res.json({
      data: await productsAPI.getInstance().getProducts(),
    });
  }
  /*
  ...
  */
  async addProducts(req: Request, res: Response) {
    const newItem = await productsAPI.getInstance().addProduct(req.body);
  ...
  */ 
  async updateProducts(req: Request, res: Response) {
    const {id} = req.params;
    const updatedProduct= await productsAPI.getInstance().updateProduct(id, req.body)
    return res.json({
  ...
  */ 
  async deleteProducts(req: Request, res: Response) {
    ...
    const deletedProduct=await productsAPI.getInstance().deleteProduct(id);
